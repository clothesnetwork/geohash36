...
task :environment do
  ...
  # # Get libs we need
  # require_relative '../../src/retina'
  # require_relative '../../src/lib/config_loader'
  # require_relative '../../src/models/init'

  # retina          = nil

  # Dir.chdir( "src" ) do
  #   # Load config.ru to get CLI opts
  #   config_loader = ConfigLoader.new
  #   variables     = config_loader.variables

  #   # Load retina app
  #   options       = Retina.new.parse_cmd_arguments( variables )
  #   retina        = Retina.new( options )
...
namespace :db do
  ...
  task :load_migrations => :environment do
    ...
    FileList['./src/models/migrations/*.rb'].each do |migration|
      load migration
    end
  end
  ...
  namespace :seed do 
    ...
    task :production => :environment do |t, _|
      puts "=> Loading seed data - Production"
      seed_file = File.expand_path('./src/models/seeders/general.rb', File.dirname(__FILE__))
      load(seed_file)
      puts "<= #{t.name} done"
    end
  ...
  desc 'Generate new migration file in src/models/migrations' # {{{
  task :new_migration, :migration_name do |_, args|
    timestamp = Time.now.strftime('%Y%m%d%H%M%S')
    name = args[:migration_name].strip.gsub(/\s+/, '_')

    content = <<EOS
DataMapper::MigrationRunner.migration #{timestamp}, :#{name} do
  up do
...
EOS
    ...
    file = File.expand_path("#{timestamp}_#{name}.rb", './src/models/migrations')

    File.write file, content

    puts "Created new migration at #{file}"
  end # }}}
